...
export const CategoryNavigation: React.FC<CategoryNavigationProps> = ({ categories }) => {
  ...
  // These are the same categories as in CategoryNavBar but with proper mapping to CategoryItem interface
  ...
  const menuItems = [
    { title: '인기 여행지', items: ['서울', '부산', '제주', '강원', '경주'] },
    { title: '테마 여행', items: ['바다', '산', '온천', '도심', '리조트'] },
    { title: '시즌 특가', items: ['여름 휴가', '겨울 스키', '벚꽃 시즌', '가을 단풍', '황금연휴'] },
  ];
    ...
    <section className="w-full my-4 relative">
      <div className="flex justify-between items-center mb-4">
        <button 
          ...
          className="flex justify-center items-center p-3 bg-white rounded-full shadow-sm border border-gray-200" 
          aria-label="Menu"
        >
        ...
        {/* Menu dropdown */}
        {menuOpen && (
          <div className="absolute top-14 left-0 w-64 bg-white rounded-lg shadow-lg z-10 p-4">
      ...
      <div className="grid grid-cols-5 md:grid-cols-10 gap-4">
        {enhancedCategories.map((category, index) => (
          ...
          >
            <div className="flex items-center justify-center w-16 h-16 rounded-full bg-white shadow-sm border border-gray-100 mb-2">
              {category.icon}
            </div>
